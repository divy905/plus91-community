...
Class BlogCategoryController extends Controller
{
	...
	public function index(Request $request){
        ...
        if($method == 'post' || $method == 'POST'){
            $rules = [];
            // $rules['category_id'] = 'required|unique:blog_category,category_id';
            ...
            $dbArray['category_name'] = $request->category_name??'';
            ...
            $dbArray['slug'] = $slug??'';
            $dbArray['status'] = 1;
            DB::table('blog_category')->insert($dbArray);
        }


        $blog_category = DB::table('blog_category')->orderBy('id','desc');

        $blog_category = $blog_category->paginate(10);
    ...
    public function delete(Request $request){
        ...
        if(is_numeric($id) && $id > 0){
            $is_delete = DB::table('blog_category')->where('id', $id)->delete();
        }

        if(!empty($is_delete)){
            return back()->with('alert-success', 'Blog Category has been deleted successfully.');
        }
        else{
    ...
    public function change_blog_category_status(Request $request){
      ...
      if(!empty($data)){
       ...
       DB::table('blog_category')->where('id',$id)->update(['status'=>$status]);
